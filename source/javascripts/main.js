...
(function( hata, window, undefined ) {
  ...
  var makeColumns = function() {
    var articles = hata(".article--index"),
      i = 1;

    // First post at left position always
    articles.get( 0 ).dataset[ "side" ] = "l";

    for ( ; i < articles.get().length; i++ ) {
      articles.get( i ).dataset[ "side" ] = articles.filter( "[data-side=l]" ).totalHeight() > articles.filter( "[data-side=r]" ).totalHeight() ? "r" : "l";
    }
  };
